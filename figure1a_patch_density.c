...
#define filename "Patch_Density_beta3.0_l0_L100000.dat"   // name of the output file
#define crack 0    // size of the initial crack
...
int broken[Ns],broken_prev[Ns],BF,NNR,NNL,D_NNR,D_NNL,D_TOT,P1;
...
int main(){
	...
	for(sample=0;sample<10000;sample++){   // configuarations
		for(Number=0;Number<Number_Max;Number++){Patch[Number]=0;Broken[Number]=0;}    // initialization
		Number=0;
		...
		for(Number=0;Number<Number_Max;Number++){Broken_Tot[Number]+=Broken[Number];}
		for(Number=0;Number<Number_Max;Number++){Patch_Tot[Number]+=Patch[Number];}
		}
	for(Number=0;Number<Number_Max;Number++){Patch_Tot[Number]=(double)Patch_Tot[Number]/Q[Number];}
	for(Number=0;Number<Number_Max;Number++){Broken_Tot[Number]=(1.0*Broken_Tot[Number])/(1.0*Q[Number]);}
	fp=fopen(filename,"a");
      	for(Number=0;Number<Number_Max;Number++){if(Patch_Tot[Number]>0){fprintf(fp,"%d\t%f\t%f\n",Number,Broken_Tot[Number],Patch_Tot[Number]);}}
       	fclose(fp);   
    	return 0;
	}
