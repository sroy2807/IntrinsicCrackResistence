...
#define filename "Burst_Distribution_beta0.5_L100000.dat"   // name of the output file
#define crack 70    // length of the pre-existing crack
...
int nnl[Ns], nnr[Ns],Pmax,NN,R,Avalanche;
...
int main(){
	...
	Nsurvivors_tot=0;
	for(Avalanche=0;Avalanche<Ns;Avalanche++){P[Avalanche]=0;}     // initialization
	for(sample=0;sample<100000;sample++){     // configuarations
		T_Fail=0;
		T_Max=0;
	...
	fp=fopen(filename,"a");
      	for(Avalanche=0;Avalanche<Ns;Avalanche++){fprintf(fp,"%d\t%d\n",Avalanche,P[Avalanche]);}
	fclose(fp);    
    	return 0;
	}
